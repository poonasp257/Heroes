#ifndef DBAUTHLOGINQUERY
#define DBAUTHLOGINQUERY

class DBAuthLoginQuery : public Query {
public:
	UInt64 clientId;
	...
	virtual ~DBAuthLoginQuery() {}

	void doResponse(SQLHSTMT statement) {
		DBAuthLoginResponsePacket packet;
		packet.accountId = 0;
		packet.clientId = clientId;
		packet.errorCode = -1;

		SQLBindCol(statement, 1, SQL_C_UBIGINT, &packet.accountId, sizeof(packet.accountId), NULL);
...
class DBAuthRegisterQuery : public Query {
public:
	UInt64 clientId;
	...
	virtual ~DBAuthRegisterQuery() {}

	void doResponse(SQLHSTMT statement) {
		DBAuthRegisterResponsePacket packet;
		packet.clientId = clientId;
		packet.errorCode = -1;
